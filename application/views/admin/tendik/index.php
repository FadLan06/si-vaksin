      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card card-primary card-outline">
            <div class="card-body">
              <button onclick="location.href='<?= base_url('Tendik/Tambah') ?>'" class="btn btn-primary btn-sm"><i class="fas fa-plus-circle"></i> Tambah</button>
              ...
              <div class="table-responsive">
                <table id="tableTendik" class="table table-sm table-bordered">
                  <thead>
                    <tr align="center">
                      <th width="5%">#</th>
                      <th width="20%">ID</th>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <a href="<?= base_url('assets/uploaddataexceltendik.xls') ?>" class="btn btn-success"><i class="fas fa-file-excel"></i> Download Format Excel</a>
        <?= form_close(); ?>
      </div>
    </div>
  </div>
...
<script>
  window.addEventListener('load', get_tendik);

  function get_tendik() {
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
      if (xhttp.readyState == 1 || xhttp.readyState == 2 || xhttp.readyState == 3) {
        document.getElementById("get_tendik").innerHTML = "<tr><td colspan='6'>Memuat data...</td></tr>";
      }

      if (xhttp.readyState == 4) {
        document.getElementById("get_tendik").innerHTML = this.responseText;
        $('#tableTendik').DataTable();
      }
    };
    xhttp.open("GET", "<?= base_url('tendik/get_tendik') ?>", true);
    xhttp.send();
  }
</script>